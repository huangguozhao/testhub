...
class ElementViewSet(viewsets.ModelViewSet):
    ...
    def import_template(self, request):
        ...
        try:
            ...
            return Response({
                'success': False,
                'error': '请先安装 openpyxl: pip install openpyxl'
        ...
        headers = [
            'name',           # 元素名称 (必填)
            'locator_strategy',  # 定位策略 (必填)
            'locator_value',   # 定位表达式 (必填)
            'page',            # 所属页面
            'element_type',    # 元素类型
            'description',      # 描述
            'component_name',   # 组件名称
        ...
        # 添加示例数据
        examples = [
            ['登录按钮', 'css', '#login-btn', '登录页面', 'BUTTON', '点击登录', 'LoginForm', 5],
            ['用户名输入框', 'xpath', '//input[@name="username"]', '登录页面', 'INPUT', '输入用户名', 'LoginForm', 5],
            ['搜索输入框', 'css', '.search-input', '搜索页面', 'INPUT', '搜索内容输入框', 'SearchBox', 3],
        ]
        ...
        ws.cell(row=7, column=1, value='# name, locator_strategy, locator_value 为必填字段')
        ws.cell(row=8, column=1, value='# locator_strategy 可选值: css, xpath, id, name, class_name, tag_name, link_text, partial_link_text')
        ws.cell(row=9, column=1, value='# element_type 可选值: INPUT, BUTTON, LINK, DROPDOWN, CHECKBOX, RADIO, TEXT, IMAGE, CONTAINER, TABLE, FORM, MODAL')
        ...
        response['Content-Disposition'] = 'attachment; filename="element_import_template.xlsx"'
        return response
    ...
    def batch_import(self, request):
        ...
        if not project_id:
            return Response({'success': False, 'error': '缺少 project_id'}, status=status.HTTP_400_BAD_REQUEST)

        # 验证项目存在且用户有权限
        try:
            project = UiProject.objects.get(id=project_id)
            user = request.user
            if project.owner != user and user not in project.members.all():
                return Response({'success': False, 'error': '无权访问此项目'}, status=status.HTTP_403_FORBIDDEN)
        except UiProject.DoesNotExist:
            return Response({'success': False, 'error': '项目不存在'}, status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            if filename.endswith('.xlsx'):
                ...
                for row in ws.iter_rows(min_row=2):
                    ...
                    result = self._parse_element_row(row_data, project, request.user, row_num)
                    if result.get('error'):
                        errors.append(result)
                    else:
                        elements_to_create.append(result['data'])
                ...
                for row in reader:
                    row_num += 1
                    result = self._parse_element_row(row, project, request.user, row_num)
                    if result.get('error'):
                        errors.append(result)
                    else:
                        elements_to_create.append(result['data'])
            else:
                return Response({'success': False, 'error': '不支持的文件格式，请上传 .xlsx 或 .csv 文件'},
                                status=status.HTTP_400_BAD_REQUEST)

        except Exception as e:
            logger.error(f"解析文件失败: {e}", exc_info=True)
            return Response({'success': False, 'error': f'解析文件失败: {str(e)}'},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        try:
            if elements_to_create:
                ...
                created_count = len(elements_to_create)
                logger.info(f"批量导入元素成功: {created_count} 个")

        except Exception as e:
            logger.error(f"批量创建元素失败: {e}", exc_info=True)
            return Response({'success': False, 'error': f'批量创建失败: {str(e)}'},
                            status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def _parse_element_row(self, row_data, project, user, row_num):
        """解析单行元素数据"""
        # 必填字段验证
        name = row_data.get('name') or row_data.get('Name') or row_data.get('元素名称')
        locator_strategy = row_data.get('locator_strategy') or row_data.get('Locator Strategy') or row_data.get('定位策略')
        locator_value = row_data.get('locator_value') or row_data.get('Locator Value') or row_data.get('定位表达式')

        if not name:
            return {'error': f'第{row_num}行: 缺少必填字段 name', 'row': row_num}
        if not locator_strategy:
            return {'error': f'第{row_num}行: 缺少必填字段 locator_strategy', 'row': row_num}
        if not locator_value:
            return {'error': f'第{row_num}行: 缺少必填字段 locator_value', 'row': row_num}

        # 查找或创建定位策略
        strategy = None
        for s in LocatorStrategy.objects.filter(name__iexact=str(locator_strategy)):
            strategy = s
            break

        if not strategy:
            return {'error': f'第{row_num}行: 定位策略 "{locator_strategy}" 不存在', 'row': row_num}

        # 根据 page 查找或创建分组
        page = row_data.get('page') or row_data.get('Page') or row_data.get('所属页面')
        group = None
        if page:
        ...
        # 解析可选字段
        element_type = row_data.get('element_type') or row_data.get('Element Type') or row_data.get('元素类型') or 'BUTTON'
        description = row_data.get('description') or row_data.get('Description') or row_data.get('描述') or ''
        component_name = row_data.get('component_name') or row_data.get('Component Name') or row_data.get('组件名称') or ''

        wait_timeout = 5
        wt = row_data.get('wait_timeout') or row_data.get('Wait Timeout') or row_data.get('等待超时')
        if wt:
            try:
                wait_timeout = int(wt)
        ...
        # 处理 element_type，确保是大写形式
        element_type = element_type.upper() if element_type else 'BUTTON'
