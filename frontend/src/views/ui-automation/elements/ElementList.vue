<template>
  <div class="page-container">
    <div class="page-header" style="display: flex; align-items: center;">
      ...
      <el-button type="success" @click="handleBatchImport" :disabled="!projectId">
        <el-icon><Upload /></el-icon>
        {{ $t('uiAutomation.element.batchImport') }}
      </el-button>
    ...
    <!-- 批量导入对话框 -->
    <el-dialog v-model="showImportDialog" :title="$t('uiAutomation.element.batchImport')" width="500px" :close-on-click-modal="false">
      <div class="import-container">
        <el-alert type="info" :closable="false" style="margin-bottom: 20px;">
          <template #title>
            {{ $t('uiAutomation.element.importTip') }}
          </template>
        </el-alert>

        <el-button type="primary" text @click="handleDownloadTemplate" style="margin-bottom: 20px;">
          <el-icon><Download /></el-icon>
          {{ $t('uiAutomation.element.downloadTemplate') }}
        </el-button>
        ...
        >
          ...
          <div class="el-upload__text">
            {{ $t('uiAutomation.element.dropFile') }} <em>{{ $t('uiAutomation.element.clickUpload') }}</em>
          </div>
          <template #tip>
            <div class="el-upload__tip">{{ $t('uiAutomation.element.uploadTip') }}</div>
          </template>
        </el-upload>

        <div v-if="importResult" class="import-result" style="margin-top: 20px;">
          <el-alert :type="importResult.success ? 'success' : 'error'" :title="importResult.success ? $t('uiAutomation.element.importSuccessTitle') : $t('uiAutomation.element.importFailedTitle')">
            <template #default>
              <div v-if="importResult.success">
                {{ $t('uiAutomation.element.importedCount') }}: {{ importResult.created_count }} / {{ importResult.total_rows }}
              </div>
              <div v-else>{{ importResult.error }}</div>
              <div v-if="importResult.errors && importResult.errors.length > 0" style="margin-top: 10px;">
                <div v-for="(err, idx) in importResult.errors.slice(0, 5)" :key="idx" style="font-size: 12px; color: #f56c6c;">
                  {{ err.error }}
                </div>
                <div v-if="importResult.errors.length > 5" style="font-size: 12px; color: #909399;">
                  ... {{ $t('uiAutomation.element.andMore', { count: importResult.errors.length - 5 }) }}
                </div>
              </div>
            </template>
          </el-alert>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="handleImport" :loading="importing" :disabled="!importFile">{{ $t('uiAutomation.common.confirm') }}</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
import { Plus, Search, View, Edit, Delete, Upload, UploadFilled, Download } from '@element-plus/icons-vue'
import {
  getUiProjects,
  getElements,
  createElement,
...
const handleImport = async () => {
  if (!importFile.value) {
    ElMessage.warning(t('uiAutomation.element.messages.selectFile'))
    return
  }

  importing.value = true
  try {
    const response = await batchImportElements(projectId.value, importFile.value)
    importResult.value = response.data
    if (response.data.success) {
      ElMessage.success(t('uiAutomation.element.messages.importSuccess', { count: response.data.created_count }))
      loadElements()
    } else {
      ElMessage.error(response.data.error || t('uiAutomation.element.messages.importFailed'))
    }
  } catch (error) {
    console.error('批量导入失败:', error)
    ElMessage.error(t('uiAutomation.element.messages.importFailed'))
  } finally {
...
const handleDownloadTemplate = () => {
  // 触发下载模板文件
  window.open('/api/ui-automation/elements/import_template/', '_blank')
}
